...
function QuestionCard({ questionNumber, editData, isEditing, onSave, onCancelEdit }) {
  ...
  useEffect(() => {
    if (editData) {
      ...
      setOptions(mapped)
      setCorrectIds(mapped.filter((o) => editData.correctAnswers.includes(o.text)).map((o) => o.id))
  ...
  return (
    <div className={`panel ${isEditing ? 'panel-editing' : ''}`}>
      <div className="panel-header space-between">
        <h3>{isEditing ? `Editing Q${questionNumber}` : `New Question — Q${questionNumber}`}</h3>
